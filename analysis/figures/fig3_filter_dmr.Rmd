...
##先算D-45和H-dmr的交集
# intersection <- intersect(D_45,H_dmr)
# union <- union(D_45,H_dmr)
# only_dmr <- setdiff(D_45,H_dmr)
# only_45 <- setdiff(H_dmr,D_45)
```


## fig3j do plot for 450 and h-dmr 
```{r}
# pdf(paste0(fig_dir,"450_filter_dmr_vennplot.pdf"))
venn.plot <- venn.diagram(dmr_list[1:2], NULL,fill=c("#E41A1C","#377EB8"),
             alph=c(0.5,0.5),
             resolution = 300,
             cex=3,
             cat.fontface=3,
             category.names = c("450K","annotated gene"))
grid.draw(venn.plot)
# dev.off()
             
```
## fig3j do plot for GC and h-dmr
```{r}
...
## fig3j do pie plot for the overalp or outside between EV-DNA and other database
```{r}
...
## fig3I-J calculate overlap between EV-DNA DMRs and GC DMRs
```{r}
...
## fig3B lolipop plot for smp gene elements

### PART 1.1 fisher result dat from shell which sorted in all dmr 
### /home/jiaozhenna/methylation/project/EM_seq/analysis/DMR_analysis/result/annotation_dmr/dmr_element
```{r}
...
# p1 <- ggplot(dat_smp,aes(x=elments,y = logp))+
#             geom_col(width = 0.5,fill= "#0073C2FF")+
#             coord_flip() +
...
```{r}
p2 <- ggplot(dat_smp, aes(x = elments, y = logp)) +
      geom_segment(aes(x = elments, xend = elments, y = 0, yend = logp), color = "#63A6D0", linewidth = 8) +
      # geom_point(color = "#63A6D0", size = 10) +
      coord_flip() +
  theme_classic()+
    # theme(
...
# ggsave(paste0(fig_dir,"lolipop_smp_dmr_20240722.pdf"),plot = p2,width = 7,height = 4,dpi = 300)
print(p2)

```


### 1.1 fisher result dat from shell which sorted in scstage dmr region
...
# p3 <- ggplot(dat_sc,aes(x=elments,y = logp))+
#             geom_col(width = 0.5,fill= "#0073C2FF")+
#             coord_flip() +
#             theme(panel.background = element_rect(fill = "transparent"),
#                   axis.line = element_line(color = "black", linewidth = 0.6),
...
## fig3F do gene heatmap and gene vilolin plot(The matrix are dmr regions between GC and GP which associated nearby genes)

### import data 
```{r}
# gene_dat <- "/home/jiaozhenna/methylation/project/EM_seq/analysis/DMR_analysis/data/fig3/"
...
### do heatmap for genes (the difference between GC and GP p-value < 0.05 ,and remove na)
...
# ###
# row_ann <- p_data[,1:5]
# row_ann$log_p <- -log(row_ann$p_adjust,10)
# ## 在注释的时候，列名要与input一致
# # col_h <- list(Element = c("enhancer" = "cadetblue","promoter" ="beige","GeneBody" = "darkseagreen"))
# # row_h <- HeatmapAnnotation(Element=row_ann$element,which="row",col=col_h)
# ## annotate p_value FDR
# col_p <- colorRamp2(c(0,5), c("white","skyblue"))
# row_p <- HeatmapAnnotation(log10p=row_ann$log_p,which = "row",col = list(log10p=col_p))
# 
# ## annotate column
# type <- list(Type=c("GP"="seagreen3","GC"="seagreen"))
# ann_type <- HeatmapAnnotation(Type=meta_df$sc_type,col = type)
# 
# gender <- list(Gender=c("Male"="thistle","Female"="steelblue"))
# ann_gender <- HeatmapAnnotation(Gender=meta_df$sc_gender,col = gender)
# 
# status <- list(Status=c("Control"="aquamarine1","Early Stage"="aquamarine3",
#                         "Advanced Stage"="aquamarine4"))
# ann_status <- HeatmapAnnotation(Status=meta_df$sc_stage,col = status)
# 
# # 绘制热图
# pheatmap::pheatmap(h_data, cluster_rows = T, 
#                    cluster_cols = FALSE,
#                   color = colorRampPalette(c("skyblue", "yellow", "orange", "red"))(100),
#                   annotation_row =row_p,
#                   annotation_col =c(ann_gender,ann_type,ann_status)
#                    )
...
# pdf(paste0(fig_dir,"fig3F_filter_DMR_heatmap_smp_gene_p.pdf"),width = 12,height = 10)
pheatmap::pheatmap(h_data,cluster_rows = T ,
...
##fig3F supplementary P值小的，小提琴图
```{r}
vil_data <- h_data%>%t()%>%as.data.frame()
vil_data$smp_status <- meta_df$sc_type

df_1 <- vil_data[,c(1,22)]
df_FAM184A <- vil_data[,c(14,22)]


## do vilolin plot
p1 <- ggplot(df_FAM184A, aes(x = smp_status,y =FAM184A)) +
      geom_violin(aes(fill=smp_status,alpha=1),
                  trim = FALSE, lwd = 0.8,width=0.8,color="white")+
      scale_fill_manual(values =c("#FC9595", "#C4E3FF"))+
      geom_boxplot(aes(fill=smp_status),outlier.shape = NA,lwd=0.4,width=0.02)+
      scale_fill_manual(values =c("#FB4F4F", "#93CDFF"),guide = 'none')+
      # geom_jitter(position=position_jitter(0.1), aes(colour=smp_status))+
      # scale_color_manual(values=c("#999999", "#E69F00"))+
      # geom_text_repel(aes(label = smp),max.overlaps = 20)+
      # geom_text(position = position_jitter(0.1), aes(colour = smp_status), size = 3, vjust = -0.5) +
      theme(axis.text.x = element_text(angle = 45,size = 15),
              axis.text.y = element_text(size = 15),
              axis.text = element_text(size = 15,colour = "black"))+
        theme_classic()#+
        # scale_color_manual(values=c("#008B8B","darkmagenta"))
      # labs(x = paste("x_", col), y = paste("y_", col), title = paste("region -", col))


        # geom_signif(comparisons = list(c("gp", "sc")),
              # textsize = 6,annotations = c(paste0("p = ", round(p.adjust, 10))),
              # y_position = 1.2,vjust = -0.5)
        # stat_compare_means(comparisons = list(c("gp", "sc")), label.y = 1,vjust=-0.5,method = "t.test",p.adjust.method="BH")
# ggsave(paste0(fig_dir,"violin_plot_FAM184A_1.pdf"),plot = p1,width = 6,height = 4,dpi = 300)
p1      
  

df_ADGRD1 <- vil_data[,c(16,22)]

p2 <- ggplot(df_ADGRD1, aes(x = smp_status,y =ADGRD1)) +
      geom_violin(aes(fill=smp_status,alpha=1),
                  trim = FALSE, lwd = 0.8,width=0.8,color="white")+
      scale_fill_manual(values =c("#FC9595", "#C4E3FF"))+
      geom_boxplot(aes(fill=smp_status),outlier.shape = NA,lwd=0.4,width=0.02)+
...
# ggsave(paste0(fig_dir,"violin_plot_ADGRD1_new.pdf"),plot = p2,width = 6,height = 4,dpi = 300,)

p2

```

### other gene violin plot
```{r}
for (gene_name in colnames(vil_data[,1:21])) {
  df <- vil_data[, c(gene_name, colnames(vil_data)[22])]
  p4 <- ggplot(df, aes(x = df[, 2], y = df[[gene_name]])) +
    geom_violin(aes(fill=smp_status,alpha=1),
                trim = FALSE, lwd = 0.8, width = 0.8,color="white") +
    scale_fill_manual(values =c("#FC9595", "#C4E3FF"))+
    geom_boxplot(aes(fill=smp_status),
                 outlier.shape = NA,lwd=0.4,width=0.02)+
    scale_fill_manual(values =c("#FB4F4F", "#93CDFF"),guide = 'none')+
    # geom_boxplot(outlier.shape = NA, lwd = 0.4, width = 0.02) +
    # geom_jitter(position = position_jitter(0.1), aes(colour = df[, 2])) +
    theme_classic()+
    ...
    # scale_color_manual(values = c("#008B8B", "darkmagenta"))
  ggsave(paste0(fig_dir,"new_violin_plot_",gene_name,".pdf"),plot = p4,width = 6,height = 4,dpi = 300)
  print(p4)
  # dev.off()
}
...
## 将venn plot intersection de gene do violin plot
 
```{r}
##import data
ann_dir <- "/home/jiaozhenna/methylation/project/EM_seq/analysis/DMR_analysis/data/fig3/"
TCGA_dmr_inter <- read_excel(paste0(ann_dir,"450K-DMR_overlap.xlsx"),sheet = "450K-DMR_overlap")
TCGA_dmr_without <- read_excel(paste0(ann_dir,"450K-DMR_overlap.xlsx"),sheet = "DMR_without")

## merge data do plot for the smallest p value plot 

TCGA_dmr_inter_df <- merge(TCGA_dmr_inter,meth_dmr,by="gene")

TCGA_dmr_inter_df$p_adjust <- as.numeric(TCGA_dmr_inter_df$p_adjust)
TCGA_dmr_inter_df <- TCGA_dmr_inter_df[!is.na(TCGA_dmr_inter_df$p_adjust),]

min(TCGA_dmr_inter_df$p_adjust)
# write.csv(TCGA_dmr_inter_df,paste0(ann_dir,"TCGA_dmr_inter_df.csv"),)

## THEN GO TO /home/jiaozhenna/methylation/project/EM_seq/analysis/DMR_analysis/code/gene_anlyais/vilolin_gene_an.Rmd
#TO DO PLOT 

TCGA_dmr_without_df <-merge(TCGA_dmr_without,meth_dmr,by="gene")

TCGA_dmr_without_df$p_adjust <-as.numeric(TCGA_dmr_without_df$p_adjust) 
min(TCGA_dmr_without_df$p_adjust)
# write.csv(TCGA_dmr_without_df,paste0(ann_dir,"TCGA_dmr_without_df.csv"))
...
```{r}
GC_dmr_inter <- read_excel(paste0(ann_dir,"GC-DMR_overlap.xlsx"),sheet = "GC_DMR_overlap")
GC_dmr_without <- read_excel(paste0(ann_dir,"GC-DMR_overlap.xlsx"),sheet = "DMR_without")

## 20 gene
GC_dmr_inter_df <- merge(GC_dmr_inter,meth_dmr,by="gene")
GC_dmr_inter_df$p_adjust <- as.numeric(GC_dmr_inter_df$p_adjust)
min(GC_dmr_inter_df$p_adjust)## TMEM132C col="chr12_127070711_127070914"
# write.csv(GC_dmr_inter_df,paste0(ann_dir,"GC_dmr_inter_df.csv"))

## 534个基因
GC_dmr_without_df <- merge(GC_dmr_without,meth_dmr,by="gene")
GC_dmr_without_df$p_adjust <- as.numeric(GC_dmr_without_df$p_adjust)
GC_dmr_without_df <- GC_dmr_without_df[!is.na(GC_dmr_without_df$p_adjust),]

min(GC_dmr_without_df$p_adjust)
# write.csv(GC_dmr_without_df,paste0(ann_dir,"GC_dmr_without_df.csv"))
```
...
```
## fig3c  do pca for dmr between GC and GP based on dmr region's methylation level (the matrix remove missing)
...
## fig3b 20241107 modified  do pca for dmr between male and female based on dmr region's methylation level (the matrix remove missing) 
```{r,fig.width=10,fig.height=8}
# import data
sample_DMR_meth_nona <- sample_DMR_meth%>%na.omit()
data <- sample_DMR_meth_nona
...
tr_data$sample_state <- meta_df$sc_type %>% as.factor()
tr_data$sample_stage <- meta_df$sc_stage  %>% as.factor()
## add genger information
tr_data$sample_gender <- meta_df$sc_gender %>% as.factor()
# calculate pca
tr.pca_all <- prcomp(tr_data[,1:28])
# do plot for pc1 and pc2
df1 <- tr.pca_all$x
pc_one <- as.data.frame(df1,tr_data$sample_state)
pc_one$Type <- tr_data$sample_state
pc_one$gender <- tr_data$sample_gender
pc_one$smp_name <- substr(rownames(tr_data),1,4)
#Extract the variance contribution rate of the principal components and generate axis titles；
summ<-summary(tr.pca_all)
xlab<-paste0("PC1(",round(summ$importance[2,1]*100,2),"%)")
ylab<-paste0("PC2(",round(summ$importance[2,2]*100,2),"%)")
p2<-ggplot(data = pc_one,aes(x=PC1,y=PC2,color=gender))+
  geom_point(size=5)+
  labs(x=xlab,y=ylab)+
  scale_color_manual(values = c("#AACBFE", "#DE8686"))+
  guides(fill=F)+
  theme_classic()+
 theme(axis.text.x = element_text(size = 15),
              axis.text.y = element_text(size = 15),
...
# ggsave(paste0(fig_dir,"pca_gender_dis_20241107.pdf"),plot = p2,width = 8,height = 5,dpi = 300)
print(p2)
```
...
res_RF_regression_list <- list()
  DMRpredict_RF_regression <- function(mat,i){
        #ratio_meth <-  readRDS(paste0(dmr_fold_proj,"dss_result_fold_",i,"/dmr_fold_",i,"_meth_matrix.rds"))%>%na.omit()
        ...
        # real_label_train <- meta$status[meta$samples %in% intrain_smp] %>% as.factor()
        # real_label_test  <- meta$status[meta$samples %in% intest_smp] %>% as.factor()
        # smp_intrain_label <- meta$smp[meta$samples %in% intrain_smp]
        ...
        RF_ob <- randomForest(smp_stage ~ .,
                      ...
                      maxnodes=10)
                      # nodesize=1,mytry=10)## mytry default is sqrt(p)
        
        varImpPlot(RF_ob)
        #saveRDS(ob1,paste0("/home/jiaozhenna/methylation/project/EM_seq/analysis/DMR_analysis/result/",i,"_test.ob1.rds")
        #pred_train1 <- predict(RF_ob,newdata = ratio_meth[intrain_id,],type = "response" )
        ...
        # roc <- roc(as.ordered(real_label_test),as.ordered(pred_test1))
        # names(pred_test1) <- rownames(intest_id)
        # 
        # pred_label_test1  <- factor(predict(ob1, newx=as.matrix(intest_id), type="class"),levels=c(0,1))##
        ...
        ## do plot for auc_test 
        # plot(AUC_test2,print.auc=T,auc.polygon=T,grid=c(0.1,0.02),grid.col=c("green","red"),
        #      ax.auc.polygon=T,auc.polygon.col="skyblue",print.thres=T)  
...
print(p1)
  ...
  # ggsave(paste0(fig_dir,"fig3E_prediction_score.pdf"),plot = p2,width = 8,height = 6,dpi = 300) 
  ...
  # roc1 <- roc(pred_dat$label, pred_dat$pred_test_cvglm,
  #           levels=c("0", "1"))
  # P3 <- plot(roc1)
  # thr.obj <- ci.thresholds(roc1) 
  # P4 <- plot(thr.obj) # ci.thresholds()
  # print(P4)
  # sp.obj <- ci.sp(roc1, sensitivities=seq(0, 1,.1), boot.n=100) 
  # P5 <- plot(sp.obj, type="shape", col="#6E8B3D") 
  # print(P5)
 }
...
for (i in 1:length(folds)){
   ...
   for (i_sample in samples){
      ...
      #for (val in thr_val) {
      data <- dmr_cov
      #data1 <- dmr_cov[rowSums(data[,2:39] < 100 & data[,2:39] > 5) == 38 , ]
      ...
      # res_list[[i]] <- DMRpredict_cv(mat = ratio_met,
                                     # i=i)
      # res_lasso_list[[i]] <- DMRpredict_lasso(mat = ratio_meth,i=i)
      # res_RF_list[[i]] <- DMRpredict_RF(mat = ratio_meth,i=i)
